              ...
              {editingBlogId === blog.id && blogForm && (
                <div className="border-t border-stone-100 px-4 py-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <label className={`flex items-center gap-2 ${canEdit ? 'cursor-pointer' : 'cursor-default'}`}>
                        <input type="checkbox" checked={blogForm.is_published} onChange={e => canEdit && setBlogForm(p => p ? { ...p, is_published: e.target.checked } : p)} disabled={!canEdit} />
                  ...
                  <div>
                    ...
                    <select value={blogForm.display_order} onChange={e => canEdit && setBlogForm(p => p ? { ...p, display_order: e.target.value } : p)}
                      className="text-xs border border-stone-200 px-2 py-1.5 focus:outline-none" disabled={!canEdit}>
