...
function toEditForm(r: Result): EditForm {
  return {
    title: r.title,
...
export default function AdminResultsPage() {
  ...
  const openAddNew = () => {
    cancelEdit();
    const maxSort = results.length > 0 ? Math.max(...results.map((r) => r.sort_order)) + 1 : 1;
  ...
  const toggleActive = async (r: Result) => {
    if (!supabase) return;
    await supabase.from('home_results').update({ is_active: !r.is_active }).eq('id', r.id);
    await loadData();
  };

  const deleteResult = async (r: Result) => {
    if (!supabase) return;
    ...
    const { error } = await supabase.from('home_results').delete().eq('id', r.id);
  ...
  const uploadMedia = async (file: File, result: Result) => {
    if (!supabase) return;
    ...
    try {
      ...
      const path = `results/${result.id}/${Date.now()}.${ext}`;
      ...
      if (result.media_url) {
        const oldPath = extractStoragePath(result.media_url);
  ...
  const deleteMedia = async (result: Result) => {
    if (!supabase) return;
    if (!window.confirm('このメディアを削除しますか？')) return;
    if (result.media_url) {
      const path = extractStoragePath(result.media_url);
      if (path) await supabase.storage.from(BUCKET).remove([path]);
    }
    const { error } = await supabase
      .from('home_results')
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-lg font-medium text-stone-800 tracking-wide">施術実績 管理</h1>
        <p className="text-xs text-stone-500 mt-1">
          トップページ「施術実績」セクションの各項目を編集します。
        </p>
      </div>
            ...
            <section key={r.id} className="bg-white border border-stone-200 rounded-lg">
              ...
              <div className="flex items-center justify-between px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-stone-800 tracking-wide">{r.title}</p>
              ...
              {editingId === r.id && form && (
                <div className="border-t border-stone-100 px-5 py-5 space-y-5">
                  ...
                  <div>
                    ...
                    {form.media_url && (
                      <div className="mb-3">
                        <div className="w-32 aspect-[3/4] bg-stone-100 rounded overflow-hidden relative mb-2">
