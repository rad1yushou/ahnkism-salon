...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({ title: 'ブログ｜LABO by AHNKISM', description: '', path: `/salon/${SALON_SLUG}/blog/${blogSlug}` });
}

export default async function LaboBlogDetailPage({ params }: Props) {
  const { blogSlug } = await params;
