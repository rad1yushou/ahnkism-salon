...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({ title: 'ブログ｜nit by AHNKISM', description: '', path: `/salon/${SALON_SLUG}/blog/${blogSlug}` });
}

export default async function NitBlogDetailPage({ params }: Props) {
  const { blogSlug } = await params;
