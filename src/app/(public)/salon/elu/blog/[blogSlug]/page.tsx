...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({ title: 'ブログ｜elu by AHNKISM', description: '', path: `/salon/${SALON_SLUG}/blog/${blogSlug}` });
}

export default async function EluBlogDetailPage({ params }: Props) {
  const { blogSlug } = await params;
